...
coonDataApp.service('loadingService', function() {

  // private variable
  var _loadingObject = {
      loadingMsg: "Project Load Initialized...",
      loadingProgress: 0
  };
  
  // public API
  this.loadingObject = _loadingObject;
  
  this.setLoadingProgress = function(num) {
      this.loadingObject.loadingProgress = num;
  };
  
  this.setLoadingMessage = function(string) {
      this.loadingObject.loadingMsg = string;
  };
  
  this.getLoadingProgress = function() {
      return this.loadingObject.loadingProgress;
  };
  
  this.getLoadingMessage = function() {
      return this.loadingObject.loadingMsg;
  };
...
coonDataApp.controller('loadCtrl', function($scope, $http, $rootScope) {
    $rootScope.loadingObject = {
      loadingMsg: "Project Load Initialized...",
      loadingProgress: 0
    };
    
    $scope.loadingObject = $rootScope.loadingObject;
...
coonDataApp.controller('branchCtrl', function($scope, $http, $rootScope){
	...
	$http({
	    ...
	    $scope.loadingObject.loadingMsg = "Loading Project Biomolecules";
	    $scope.loadingObject.loadingProgress = 5;
...
$http({
	...
	{
	    $scope.loadingObject.loadingMsg = "Loading Project Biomolecules";
	    $scope.loadingObject.loadingProgress = 10;
		...
		// we have all the molecular data. Now we need to query all metadata associated with the project
		
		// define function to handle recursive post requests.
	var recursivelyQueryMetadata = function(step, iteration, offset = 0) 
	{
		    var limit = step;
		    
		    $scope.loadingObject.loadingMsg = "Loading Biomolecule Metadata";
		    $scope.loadingObject.loadingProgress = iteration * 10 - 1;
		    
		    if (iteration == 10) {
		        limit = metadataCount - offset;
		    }
		    ...
		    $http({
    			...
    			if (iteration == 10) {
    			    $scope.loadingObject.loadingMsg = "Project Loaded!";
	                $scope.loadingObject.loadingProgress = 99.9;
    			    ...
    			    $http({
            		    ...
            		    $scope.loadingObject.loadingMsg = "Project Loaded!";
                        $scope.loadingObject.loadingProgress = 100;
            	    });
    			...
    			recursivelyQueryMetadata(step, ++iteration, offset);
    		});
		};
		
		// define variable to hold our total number of metadata entries
		var metadataCount = -1;
		...
		$http({
			...
			metadataCount = response.data[0]["count(*)"];
			// now that we have the count of metadata terms
			// we will call the function 10 times recursively to get 
			// the rest of the metadata terms without bricking the site/server
			
			var step = Math.floor(metadataCount / 10);
			var iteration = 1;
			
			recursivelyQueryMetadata(step, iteration);
	    });
	} else {
	    $scope.loadingObject.loadingMsg = "Loading Complete!";
	    $scope.loadingObject.loadingProgress = 99.9;
