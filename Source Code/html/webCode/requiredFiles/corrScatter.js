 ...
 scope.$watchCollection(function() { 
    return angular.toJson([scope.$parent.highLightList]);}, function() { 
        //scope.newSelection();
        scope.highlightChange();
    });
...
{
    var curr_circles = chart_scatter.selectAll("circle");
    curr_circles[0].forEach(function(d){
     if(scope.$parent.highLightList[d.__data__.i]!==undefined)
     {
        d.__data__.c = scope.$parent.highLightList[d.__data__.i];
...
scope.highlightChange = function() {
   chart_scatter.selectAll("circle").style("fill", function(d) {
    ...
    if(scope.$parent.highLightList[d.i]!==undefined)
    {
        d.c = scope.$parent.highLightList[d.i];
        d.opacity = 1;
        d.sw="2px";
    }
 ...
 lrData = [];
                      ...
                      .on("mouseover", function(d) {
                       ...
                       tip_scatter.html(function(i) {
                          ...
                          if (scope.$parent.scatterShortenLongTerms)
                          {
                         ...
                         return (namePart +
                           ...
                           "<strong style='color:dodgerblue'>" + "<span>" + scope.condYAxis + "</span>" + " FDR-adjusted Q-value: " + "</strong> <span style='color:white'>" + d.p2_fdr.toExponential(4) + " </span><br>" +
                           "<strong style='color:dodgerblue'>" + "<span>" + scope.condYAxis + "</span>" + " Bonferroni-adjusted P-value: " + "</strong> <span style='color:white'>" + d.p2_bonferroni.toExponential(4) + " </span>"
                           )
});
 ...
 .on("mouseout", function(d) {
   ...
   if (d3.select(this)[0][0].style.fill != "orange")
   {
       d3.select(this).style("fill", function(d) {
           return d.c;
       }).style("stroke-opacity", 1).style("opacity", function(d) {
           return d.opacity
       }).style("stroke-width",function(d){return d.sw; }).style("stroke", "black");
   }
   tip_scatter.hide();
})
 .on("click", function(d)
