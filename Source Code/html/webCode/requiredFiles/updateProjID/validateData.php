...
//get all POST variables
	...
	$organism_id=(int)$_POST['o'];
	$standard_identifier_column_header=$_POST['sc'];
	$standard_identifier_type=(int)$_POST['st'];
	$stat_test = $_POST['test'];
...
//add POST variables to temporary file headers table (note: once the file has been validated you need to move this information to permanent file table so it cannot be deleted or overwritten).
	$argLine = "INSERT INTO project_temporary_file_headers (
		uploader_user_id,project_id,temporary_file_id,original_file_name,file_name,upload_time,identifier,feature_descriptors,quant,log2_transform,impute_missing_values,filter,branch_id,set_name,delimiter, organism_id,standard_id_column,standard_id_type, stat_test)
VALUES (:uploader_user_id,:project_id,:temporary_file_id,:original_file_name,:file_name,:upload_time,:identifier,:feature_descriptors,:quant,:log2_transform,:impute_missing_values,:filter,:branch_id,:set_name,:delimiter, :organism_id,:standard_id_column,:standard_id_type, :stat_test)";
$query_params = array(
	...
	':impute_missing_values' => $impute,':filter' => $filter,':branch_id' => $branch_id,':set_name' => $set_name,':delimiter' => $delimiter, ':organism_id' => $organism_id, ':standard_id_column' => $standard_identifier_column_header, ':standard_id_type' => $standard_identifier_type, ':stat_test' => $stat_test
	); 
try
{
...
//Check that only one control condition is selected.
	$control_cond_count_array = array();
	$tmp_quant_obj = json_decode($quant, true);
	foreach ($tmp_quant_obj as $entry) {
		if($entry['control']==="Yes")
		{
			$control_cond_count_array[$entry['condName']]="";
		}
	}
	if (count($control_cond_count_array)>1)
	{
		$control_count_error_string = "More than one control condition selected. Please select no more than one control.";
		array_push($critical_errors, $control_count_error_string);
	}
...
//check if each header is present in $header_array
	...
	if(!empty($standard_identifier_column_header))
	{
		array_key_exists($standard_identifier_column_header, $header_array) ? null : array_push($critical_errors, "Header not found: " . $standard_identifier_column_header);
	}
...
//check that each quantitative column has at least one numeric value present
	...
	//check that organism_id and standard_identifier_type (if specified) are within the acceptable numeric ranges
	if(!is_numeric($organism_id))
	{
		array_push($critical_errors, "Invalid Organism Specified");
	}
	else
	{
		($organism_id===-1 || ($organism_id<=11&&$organism_id>=1)) ? null : array_push($critical_errors, "Invalid Organism Specified");
	}
	if(!is_numeric($standard_identifier_type))
	{
		array_push($critical_errors, "Invalid Standard Identifier Type Specified");
	}
	else
	{
		($organism_id===-1 || ($standard_identifier_type<=5&&$standard_identifier_type>=1)) ? null :array_push($critical_errors, "Invalid Standard Identifier Type Specified");;
	}
