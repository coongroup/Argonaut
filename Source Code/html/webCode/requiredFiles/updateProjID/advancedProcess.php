...
function HandleUpload($process_id, $set_id, $projectID, $db)
{
	...
	$stat_test=$file_array['stat_test'];
...
/*	$standard_id_mappings = GetStandardIDMappings($master_id_list, $organism_id, $standard_id_column, $standard_id_type, $db);*/
	...
	CalculateConditionDescriptiveStatistics($condition_dict, $log2_transform, $stat_test); UpdateProgress($process_id, (8/18)*100, $db);
...
function GetFileInformation($set_id, $projectID, $process_id, $db) //Gets all file information and returns in an array
{
	...
	$return_array = array("og_file_name" => $og_file_name, "file_name" => $file_name, "identifier" => $identifier, "feature_descriptors" => $feature_descriptors,
		...
		"set_name" => $set_name, "delimiter" => $delimiter, "file_id" => $file_id, "organism_id" => $organism_id, "standard_id_column" => $standard_id_column, "standard_id_type"=> $standard_id_type, 'set_name' => $set_name, 'stat_test' => $stat_test);
	return $return_array;
}
...
function CalculateConditionDescriptiveStatistics(&$condition_dict, $log2_transform, $stat_test)
{
	foreach ($condition_dict as $cond) {
		$complete_quant_arrays = array();
		foreach ($cond->replicate_list as $rep) {
		...
		foreach (array_keys($cond->quant_dict_all_vals) as $key) {
			...
			if (count($complete_quant_p_value_array)>0 && count($cond->quant_dict_all_vals[$key])>0)
			{
				...
				if ($stat_test == "STUDENT") {
				    ...
				    count($cond->quant_dict_all_vals[$key]) >1 && count($complete_quant_p_value_array) > 1 ? $mean_norm_p_val = BCWelchTTestTwoSample($cond->quant_dict_all_vals[$key], $complete_quant_p_value_array) : null;
				}
				...
				if (!empty($control_condition))
				{
					if (array_key_exists($key, $control_condition->quant_dict_all_vals))
					{
						...
						if ($stat_test == "STUDENT") {
						    ...
						    count($cond->quant_dict_all_vals[$key])>1 && count($control_condition->quant_dict_all_vals[$key])>1 ? $control_norm_p_value = BCWelchTTestTwoSample($cond->quant_dict_all_vals[$key], $control_condition->quant_dict_all_vals[$key]) : null;
						}
...
function AddReplicateDataToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings)
{
	...
	fclose($new_file);
	$query = "LOAD DATA LOCAL INFILE '/var/www/html/DV/" . $projectID . "/" . $set_id . "_data_replicate_data.txt' INTO TABLE data_replicate_data FIELDS TERMINATED BY '\t' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' (project_id, unique_identifier_id, quant_value, quant_value_log2, quant_value_raw, replicate_id, condition_id, set_id, branch_id, file_id, is_imputed, standard_molecule_id)";
...
function AddConditionDataToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings)
{
	...
	fclose($new_file);
	$query = "LOAD DATA LOCAL INFILE '/var/www/html/DV/" . $projectID . "/" . $set_id . "_data_condition_data.txt' INTO TABLE data_condition_data FIELDS TERMINATED BY '\t' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' (project_id, unique_identifier_id, avg_quant_value, all_quant_values, std_dev_quant_value, cv_quant_values, num_quant_values, condition_id, set_id, branch_id, file_id, standard_molecule_id)";
	$stmt = $db->prepare($query, array(PDO::MYSQL_ATTR_LOCAL_INFILE => true));
	$result = $stmt->execute();
...
function HandleErrorExit($process_id, $set_id, $projectID, $db)
{
    ...
    $mail->AddAddress("nickwiecien@gmail.com", "Nick"); //replace myname and mypassword to yours
	...
	$newFile = fopen($set_id . '-UploadSettings.txt', "w");
	...
	$query = "UPDATE process_queue SET progress=100, process_failed=1, completed=1, running=0 WHERE process_id=:process_id";
...
function BCWelchTTestTwoSample($array1, $array2)
{
	if(empty($array1) || empty($array2)) { return 1;}
	if(count($array1)===0 || count($array2)===0) { return 1;}
	$μ₁ = BCMean($array1); $μ₂ = BCMean($array2); $n₁ = count($array1); $n₂ = count($array2);
	...
	$vnum = bcpow(bcadd(bcdiv(bcpow($σ₁, 2), $n₁), bcdiv(bcpow($σ₂, 2), $n₂)), 2);
	$vdenom = bcadd(bcdiv(bcpow($σ₁, 4), bcmul(bcpow($n₁, 2), bcsub($n₁, 1))), bcdiv(bcpow($σ₂, 4), bcmul(bcpow($n₂, 2), bcsub($n₂, 1))));
