...
$insertText = "INSERT INTO project_files (project_id, uploader_user_id, original_file_name, file_name, delimiter, upload_time, impute_missing_values, log2_transform, identifier, 
	feature_descriptors, quant, filter, branch_id, set_name, organism_id, standard_id_column, standard_id_type, stat_test) VALUES (:project_id, :uploader_user_id, :original_file_name, :file_name, :delimiter, :upload_time, :impute_missing_values,
	:log2_transform, :identifier, :feature_descriptors, :quant, :filter, :branch_id, :set_name, :organism_id, :standard_id_column, :standard_id_type, :stat_test)";
$query_params = array(':project_id' => $projectID, ':uploader_user_id' => $_SESSION['user'], ':original_file_name' => $og_file_name, ':file_name' => $file_name, ':delimiter' => $delimiter,
	...
	':quant' => $quant, ':filter' => $filter, ':branch_id' => $branch_id, ':set_name' => $set_name, ':organism_id' =>$organism_id, ':standard_id_column' => $standard_id_column, ':standard_id_type' => $standard_id_type, ':stat_test' => $stat_test);
...
//increment project_max_nodes table

$query = "UPDATE project_max_nodes SET max_set_number=:max_set WHERE project_id=:project_id";
$query_params = array(':project_id' => $projectID, ':max_set'=>$max_set);
$stmt = $db->prepare($query);
$result = $stmt->execute($query_params);
...
$insertText = "UPDATE project_max_nodes SET max_condition_number=:max_cond WHERE project_id=:project_id";
$query_params = array(':max_cond'=>$max_condition, ':project_id'=> $projectID);
$stmt = $db->prepare($insertText);
	$result = $stmt->execute($query_params);
...
//update project_max_nodes
$insertText = "UPDATE project_max_nodes SET max_replicate_number=:max_rep WHERE project_id=:project_id";
$query_params = array(':max_rep'=>$max_replicate, ':project_id'=> $projectID);
$stmt = $db->prepare($insertText);
	$result = $stmt->execute($query_params);
...
$insertText = "INSERT INTO project_activity (project_id, activity, time, description) VALUES (:project_id, :activity, :time, :description)";
$query_params = array(':project_id' => $projectID, ':description' => $og_file_name . " uploaded as '" . $set_name . "' by " . $_SESSION['pref'] . " " . $_SESSION['last'],
	':time' =>  date("Y-m-d H:i:s"), ':activity' => 'UPLOAD');
try
{
	$stmt = $db->prepare($insertText);
	$result = $stmt->execute($query_params); 
...
$insertText = "INSERT INTO process_queue (user_id, project_id, set_id, branch_id, task, task_params, running, completed, task_creation_time, task_completion_time) VALUES 
(:user_id, :project_id, :set_id, :branch_id, :task, :task_params, :running, :completed, :task_creation_time, :task_completion_time)";
$query_params = array(':user_id' => $_SESSION['user'], ':project_id' => $projectID, ':set_id' => $set_id, ':branch_id' => $branch_id, ':task' => 'UPLOAD', ':task_params' => "", ':running' => 0,
