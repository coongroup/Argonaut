...
foreach ($update_data as $entry) {
	...
	$control_description = "Updated imputation settings in '" . $set_name . "' (Branch: " . $branch_name . ")";
	$query = "INSERT INTO project_activity (project_id, activity, time, description) VALUES (:project_id, :activity, :time, :description)";
	$query_params = array(':project_id' => $projectID, ':activity' => 'IMPUTATION SETTING UPDATE', ':time' => $time, ':description' => $control_description);
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);
	...
	$query_params = array(':user_id' => $entry['uploader_user_id'], ':project_id' => $projectID, ':task_params' => '', ':task' => 'IMPUTE', ':set_id' => $new_set_id, ':branch_id' => $branch_id,
