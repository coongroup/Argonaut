...
//variables for error exit handling
...
while (true)
{
	...
	global $set_info_array;
	$set_info_array['user_id'] = $row['user_id'];
	$set_info_array['set_id'] = $row['set_id'];
	$set_info_array['branch_id'] = $row['branch_id'];
	$set_info_array['task'] = $row['task'];
	$set_info_array['task_creation_time'] = $row['task_creation_time'];
	...
	try{
		switch ($task) {
			case 'UPLOAD':
			HandleUpload($process_id, $set_id, $projectID, $db, $projectPath);
			break;
	...
	{
		HandleErrorExit($process_id, $set_id, $projectID, $db);
	}
...
function HandleUpload($process_id, $set_id, $projectID, $db, $projectPath)
{
	...
	$file_info_array = $file_array;

	//you've got all of the pieces here, you should be ready to go.
	//For the docker image. you're trying to reference files in /var/tmp/ from /var/www/html/bW55gC6/
...
/*	$standard_id_mappings = GetStandardIDMappings($master_id_list, $organism_id, $standard_id_column, $standard_id_type, $db);*/
	...
	$standard_id_mappings = GetStandardIDMappings($master_id_list, $organism_id, $standard_id_column, $standard_id_type, $db, $file_array, $identifier_indexes, $feature_descriptor_header_indexes);
	...
	AddReplicateDataToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings, $projectPath); UpdateProgress($process_id, (10/18)*100, $db);
	...
	AddConditionDataToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings, $projectPath);
	AddDataDescriptiveStatisticsToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings, $projectPath); UpdateProgress($process_id, (12/18)*100, $db);
	...
	//GET ALL DATA FOR PCA
	$pca_cond_dict = GetPCAQuantDict($db, $projectID, $branch_id, false);
	...
	$pca_cond_dict = GetPCAQuantDict($db, $projectID, $branch_id, true);
	$rep_pca_results = DoPCA($pca_cond_dict, $master_id_list, true);
...
function GetFileInformation($set_id, $projectID, $process_id, $db) //Gets all file information and returns in an array
{
	...
	$return_array = array("og_file_name" => $og_file_name, "file_name" => $file_name, "identifier" => $identifier, "feature_descriptors" => $feature_descriptors,
		...
		"set_name" => $set_name, "delimiter" => $delimiter, "file_id" => $file_id, "organism_id" => $organism_id, "standard_id_column" => $standard_id_column, "standard_id_type"=> $standard_id_type, 'set_name' => $set_name);
	return $return_array;
}
...
function GetStandardIDMappings($master_id_list, $organism_id, $standard_id_column, $standard_id_type, $db, $file_array, $identifier_indexes, $feature_descriptor_header_indexes)
{
/*	//if organism id === -1 or $standard_id_column is empty or $standard_id_type===-1 return right away
	...
	foreach ($row as $entry) {
	 	$id_text_to_id_dict[strtolower($entry['id_text'])] = $entry['molecule_id'];
	}
	...
	foreach ($master_id_list as $key => $value) {
		...
		foreach ($key_list as $split_key) {
			array_key_exists(strtolower(trim($split_key)), $id_text_to_id_dict) ? $standard_id_mapping_array[$value] = $id_text_to_id_dict[strtolower(trim($split_key))] : null;
		}		
	}
	return $standard_id_mapping_array;*/
...
$standard_id_mapping_array = array();
	...
	$standard_id_index = -1;
	foreach ($identifier_indexes as $key => $value) {
		$key === $standard_id_column ? $standard_id_index = $value : null;
	}
	foreach ($feature_descriptor_header_indexes as $key => $value) {
		$key === $standard_id_column ? $standard_id_index = $value : null;
	}

	$standard_id_text_to_unique_id_dict = array();

	for($i = 1; $i < count($file_array); $i++)
	{
		//echo($file_array[$i][$standard_id_index] . "<br>");
		$currStandardIDText = $file_array[$i][$standard_id_index];
		$currUniqueIDText = $file_array[$i][$identifier_index];
		$currUniqueID = $master_id_list[$currUniqueIDText];
		$standard_id_text_to_unique_id_dict[$currUniqueID] = strtolower( $currStandardIDText);
		/*$pattern = '/[;,:|]/';
		$key_list = preg_split( $pattern, $currStandardIDText);
		foreach ($key_list as $entry) {
			$standard_id_text_to_unique_id_dict[$currUniqueID] = strtolower( $currStandardIDText);
		}*/
	}
	...
	foreach ($standard_id_text_to_unique_id_dict as $key => $value) {
		//echo(json_encode($value) . "<br>");
		if (!array_key_exists($key, $standard_id_mapping_array))
		{
			$standard_id_mapping_array[$key] = -1;
		}
		$pattern = '/[;,:|]/';
		$key_list = preg_split( $pattern, $value);
		foreach ($key_list as $split_key) {
			array_key_exists(strtolower(trim($split_key)), $id_text_to_id_dict) ? $standard_id_mapping_array[$key] = $id_text_to_id_dict[strtolower(trim($split_key))] : null;
		}		
	}
	return $standard_id_mapping_array;
...
function CreateReplicateObjects($file_array, $identifier_indexes, $master_id_list, $projectID, $db, $file_id, $quant, $quant_header_indexes, $log2_transform)
{
	...
	for ($i = 1; $i<count($file_array); $i++)
	{
		...
		foreach ($replicate_dict as $rep) {
			...
			$adj_quant_val = ($log2_transform==="1" && ($quant_val!=="0" && $quant_val!==0))? log($quant_val, 2) : $quant_val;
				...
				foreach ($cond->replicate_list as $rep) {
					...
					foreach (array_keys($rep->quant_dict) as $key) {
						...
						if ($curr_val===0)
						{
							...
							$new_val =  sample_from_norm_dist($avg, $sd);
	...
	foreach ($condition_dict as $cond) {
		...
		foreach ($cond->quant_dict_all_vals as $key => $value) {
			...
			if (count($value)>1)
			{
				...
				$cond->quant_dict_sds[$key] = standard_deviation($value);
			}
			else
			{
	...
	foreach ($condition_dict as $cond) {
		...
		foreach (array_keys($cond->quant_dict_all_vals) as $key) {
			...
			if (count($complete_quant_p_value_array)>0 && count($cond->quant_dict_all_vals[$key])>0)
			{
				...
				count($cond->quant_dict_all_vals[$key]) >1 && count($complete_quant_p_value_array) > 1 ? $mean_norm_p_val = BCTTestTwoSample($cond->quant_dict_all_vals[$key], $complete_quant_p_value_array) : null;
				//BCTTestTwoSample
				...
				if (!empty($control_condition))
				{
					if (array_key_exists($key, $control_condition->quant_dict_all_vals))
					{
						...
						count($cond->quant_dict_all_vals[$key])>1 && count($control_condition->quant_dict_all_vals[$key])>1 ? $control_norm_p_value = BCTTestTwoSample($cond->quant_dict_all_vals[$key], $control_condition->quant_dict_all_vals[$key]) : null;
...
function WriteTruncatedFile($file_array, $identifier_indexes, $feature_descriptor_header_indexes, $quant_header_indexes, $quant, $feature_descriptors, $identifier, $truncated_file_path, $file_id, $db, $project_id, $condition_dict, $master_id_list)
{
	...
	$complete_truncated_file_path = 'UploadFolder/' . $truncated_file_path;
...
function AddReplicateDataToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings, $projectPath)
{
	...
	$new_file = fopen($set_id . '_data_replicate_data.txt', "w");
	foreach ($condition_dict as $cond) {
		foreach ($cond->replicate_list as $rep) {
			$insertArray = array();
			foreach ($rep->quant_dict as $key => $value) {
				$is_imputed = 0; 
				$rep->quant_dict[$key]===$rep->quant_dict_pre_imputation[$key] ? $is_imputed = 0 : $is_imputed = 1;
				$newArray = array($projectID, $key, $value, $value, pow(2, $value), $rep->replicate_id, $cond->condition_id, $set_id, $branch_id, $file_id, $is_imputed, $standard_id_mappings[$key]);
				fwrite($new_file, implode("\t", $newArray) . "\r\n");
			}
		}
	}
	fclose($new_file);
	$query = "LOAD DATA LOCAL INFILE'" . $set_id . "_data_replicate_data.txt' INTO TABLE data_replicate_data FIELDS TERMINATED BY '\t' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' (project_id, unique_identifier_id, quant_value, quant_value_log2, quant_value_raw, replicate_id, condition_id, set_id, branch_id, file_id, is_imputed, standard_molecule_id)";
	$stmt = $db->prepare($query, array(PDO::MYSQL_ATTR_LOCAL_INFILE => true));
	$result = $stmt->execute();
	unlink($set_id . '_data_replicate_data.txt');
}
function AddConditionDataToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings, $projectPath)
{
	//TRADITIONAL INSERT METHODS
	/*foreach ($condition_dict as $cond) {
		$insertArray = array();
	...
	$new_file = fopen($set_id . '_data_condition_data.txt', "w");
	foreach ($condition_dict as $cond) {
		foreach ($cond->quant_dict_all_vals as $key => $value) {
			$newArray = array($projectID, $key, $cond->quant_dict_avg_val[$key], implode(";", $value), $cond->quant_dict_sds[$key], $cond->quant_dict_cvs[$key], count($value), $cond->condition_id, $set_id, $branch_id, $file_id, $standard_id_mappings[$key]);
			fwrite($new_file, implode("\t", $newArray) . "\r\n");
		}
	}
	fclose($new_file);
	$query = "LOAD DATA LOCAL INFILE '" . $projectPath . $set_id . "_data_condition_data.txt' INTO TABLE data_condition_data FIELDS TERMINATED BY '\t' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' (project_id, unique_identifier_id, avg_quant_value, all_quant_values, std_dev_quant_value, cv_quant_values, num_quant_values, condition_id, set_id, branch_id, file_id, standard_molecule_id)";
	$stmt = $db->prepare($query, array(PDO::MYSQL_ATTR_LOCAL_INFILE => true));
	$result = $stmt->execute();
	unlink($set_id . '_data_condition_data.txt');
}
function AddDataDescriptiveStatisticsToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings, $projectPath)
{
	//TRADITIONAL INSERT STATEMENT HERE
	/*foreach ($condition_dict as $cond) {
	...
	fclose($new_file);
	$query = "LOAD DATA LOCAL INFILE '" . $projectPath . $set_id . "_data_descriptive_statistics.txt' INTO TABLE data_descriptive_statistics FIELDS TERMINATED BY '\t' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' (project_id, file_id, set_id, branch_id, condition_id, unique_identifier_id, quant_val, fold_change_mean_norm, fold_change_control_norm, p_value_mean_norm, p_value_control_norm, std_dev, bonferroni_p_value_control_norm, bonferroni_p_value_mean_norm, fdr_p_value_control_norm, fdr_p_value_mean_norm, standard_molecule_id)";
	$stmt = $db->prepare($query, array(PDO::MYSQL_ATTR_LOCAL_INFILE => true));
	$result = $stmt->execute();
...
function GetPCAQuantDict($db, $projectID, $branch_id, $isRep)
{
	if($isRep)
	{
		$returnDict = array();
		$query = "SELECT a.quant_value, a.unique_identifier_id, a.replicate_id, a.condition_id, b.condition_name FROM data_replicate_data AS a JOIN project_conditions AS b ON a.condition_id=b.condition_id WHERE a.branch_id=:branch_id";
		$query_params = array(':branch_id' => $branch_id);
		$stmt = $db->prepare($query);
		$result = $stmt->execute($query_params);
		$row = $stmt->fetchAll();
		foreach ($row as $entry) {
			if(!array_key_exists($entry['condition_id'], $returnDict))
			{
				$currCond = new Condition();
				$currCond->condition_id = $entry['condition_id'];
				$currCond->condition_name = $entry['condition_name'];
				$returnDict[$entry['condition_id']] = $currCond;
			}
			if(!array_key_exists($entry['replicate_id'], $returnDict[$entry['condition_id']]->replicate_list))
			{
				$currRep = new Replicate();
				$currRep->replicate_id = $entry['replicate_id'];
				$returnDict[$entry['condition_id']]->replicate_list[$entry['replicate_id']] = $currRep;
			}
			$returnDict[$entry['condition_id']]->replicate_list[$entry['replicate_id']]->quant_dict[$entry['unique_identifier_id']] = $entry['quant_value'];
		}
		return $returnDict;
	}
	else
	{
		$returnDict = array();
		$query = "SELECT a.avg_quant_value, a.unique_identifier_id, a.condition_id, b.condition_name FROM data_condition_data AS a JOIN project_conditions AS b ON a.condition_id=b.condition_id WHERE a.branch_id=:branch_id";
				$query_params = array(':branch_id' => $branch_id);
		$stmt = $db->prepare($query);
		$result = $stmt->execute($query_params);
		$row = $stmt->fetchAll();
		foreach ($row as $entry) {
			...
			$returnDict[$entry['condition_id']]->quant_dict_avg_val[$entry['unique_identifier_id']] = $entry['avg_quant_value'];
		}
		return $returnDict;
	}
}
...
function HandleErrorExit($process_id, $set_id, $projectID, $db)
{
	echo("in handle error exit<br>");
	echo($set_id . "<br>");
	global $gerrstr, $gerrfile, $gerrline, $set_info_array, $file_info_array;

	$l_project_id = $projectID;
	$l_set_id = $set_id;
	$l_branch_id = array_key_exists('branch_id', $set_info_array) ? $set_info_array['branch_id'] : "";
	$l_set_name = array_key_exists('set_name', $file_info_array) ? $file_info_array['set_name'] : "";
	$l_file_id = array_key_exists('file_id', $file_info_array) ? $file_info_array['file_id'] : -1;
	$l_uploader_user_id = array_key_exists('user_id', $set_info_array) ? $set_info_array['user_id'] : -1;
	$l_og_file_name = array_key_exists('og_file_name', $file_info_array) ? $file_info_array['og_file_name'] : "";
	$l_file_name = array_key_exists('file_name', $file_info_array) ? $file_info_array['file_name'] : "";
	$l_delimiter = array_key_exists('delimiter', $file_info_array) ? $file_info_array['delimiter'] : "";
	$l_impute_missing_values = array_key_exists('impute', $file_info_array) ? $file_info_array['impute'] : -1;
	$l_log2_transform = array_key_exists('log2_transform', $file_info_array) ? $file_info_array['log2_transform'] : -1;
	$l_organism_id = array_key_exists('organism_id', $file_info_array) ? $file_info_array['organism_id'] : -1;
	$l_standard_id_column = array_key_exists('standard_id_column', $file_info_array) ? $file_info_array['standard_id_column'] : "";
	$l_standard_id_type = array_key_exists('standard_id_type', $file_info_array) ? $file_info_array['standard_id_type'] : -1;
	$l_identifier = array_key_exists('identifier', $file_info_array) ? $file_info_array['identifier'] : "";
	$l_feature_descriptors = array_key_exists('feature_descriptors', $file_info_array) ? $file_info_array['feature_descriptors'] : "";
	$l_quant = array_key_exists('quant', $file_info_array) ? $file_info_array['quant'] : "";
	$l_filter = array_key_exists('filter', $file_info_array) ? $file_info_array['filter'] : "";
	$l_error_message = $gerrstr;
	$l_error_file = $gerrfile;
	$l_error_line = $gerrline;
	$l_task = array_key_exists('task', $set_info_array) ? $set_info_array['task'] : "";
	$l_upload_time = array_key_exists('task_creation_time', $set_info_array) ? $set_info_array['task_creation_time'] : "";

	$mail = new PHPMailer();
	$mail->IsSMTP(); // set mailer to use SMTP

	$mail->From = "coonlabdata@coonlabdatadev.com";
    $mail->FromName = "Dain";
    $mail->Host = "server1.coonlabdatadev.com"; // specif smtp server
    ...
    $mail->SMTPAuth = true;
    $mail->Username = "coonlabdata@coonlabdatadev.com"; // SMTP username
    $mail->Password = "cret4afutUcu"; // SMTP password
    $mail->AddAddress("dainbrademan@gmail.com", "Dain"); //replace myname and mypassword to yours

	$mail->WordWrap = 50; // set word wrap

	$mail->IsHTML(true); // set email format to HTML
	$mail->Subject = 'File Upload Failure_' . $set_id;
	$mail->Body = 'File uploaded by user ' . $l_uploader_user_id . ' failed to upload. The file is attached with associated settings here.';

	$newFile = fopen("UploadFolder/" . $set_id . '-UploadSettings.txt', "w");
	fwrite($newFile, 'Set Name' . "\t" . $l_set_name . "\r\n");
	$identifier_obj = json_decode($l_identifier, true);
	fwrite($newFile, 'Identifier' . "\r\n");
	foreach ($identifier_obj as $entry) {
	 	fwrite($newFile, $entry['header'] . "\t" . $entry['userName'] . "\r\n");
	}
	fwrite($newFile, "\r\n");
	$feature_descriptors_obj = json_decode($l_feature_descriptors, true);
	fwrite($newFile, 'FeatureDescriptors' . "\r\n");
	foreach ($feature_descriptors_obj as $entry) {
		fwrite($newFile, $entry['header'] . "\t" . $entry['userName'] . "\r\n");
	}
	fwrite($newFile, "\r\n");
	$quant_obj = json_decode($l_quant, true);
	foreach ($quant_obj as $entry) {
		$boolval =  $entry['control']==="Yes" ? "True\r\n" : "False\r\n";
		fwrite($newFile, $entry['header'] . "\t" . $entry['condName'] . "\t" . $entry['repName'] . "\t" .$boolval);
	}
	fwrite($newFile, "\r\n");
	fwrite($newFile, 'Sample Type' . "\t" . $l_organism_id . "\r\n");
	fwrite($newFile, 'Standard ID Column' . "\t" . $l_standard_id_column . "\r\n");
	fwrite($newFile, 'Standard ID Type' . "\t" . $l_standard_id_type . "\r\n");
	fwrite($newFile, 'Filter' . "\t" . $l_filter . "\r\n");
	fwrite($newFile, 'Impute' . "\t" . $l_impute_missing_values . "\r\n");
	fwrite($newFile, 'Log2' . "\t" . $l_log2_transform . "\r\n");
	fwrite($newFile, "\r\n");
	fwrite($newFile, 'Error Message' . "\t" . $l_error_message . "\r\n");
	fwrite($newFile, 'Error File' . "\t" . $l_error_file . "\r\n");
	fwrite($newFile, 'Error Line' . "\t" . $l_error_line . "\r\n");
	fclose($newFile);

	$file_to_attach = 'UploadFolder/' . $l_file_name;
	$mail->addAttachment($file_to_attach, 'failed_file.txt');
	$mail->addAttachment($set_id . '-UploadSettings.txt', 'failed_file_settings.txt');
	$mail->send();

	unlink("UploadFolder/" . $set_id . '-UploadSettings.txt');

	//add new project_failed_sets entry
	$query = "INSERT INTO project_failed_sets (project_id, set_id, branch_id, set_name, file_id, uploader_user_id, original_file_name, file_name, delimiter, upload_time, impute_missing_values, log2_transform, organism_id, standard_id_column, standard_id_type, " 
		. "identifier, feature_descriptors, quant, filter, error_message, error_file, error_line, task) VALUES (:project_id, :set_id, :branch_id, :set_name, :file_id, :uploader_user_id, :original_file_name, :file_name, :delimiter, :upload_time, :impute_missing_values, :log2_transform, :organism_id, :standard_id_column, :standard_id_type, " 
		. ":identifier, :feature_descriptors, :quant, :filter, :error_message, :error_file, :error_line, :task)";
	$query_params = array(':project_id' => $l_project_id,
	 ':set_id' => $l_set_id, ':branch_id'=> $l_branch_id, ':set_name' => $l_set_name, ':file_id' => $l_file_id, ':uploader_user_id' => $l_uploader_user_id, ':original_file_name' => $l_og_file_name, ':file_name' => $l_file_name,
	  ':delimiter' => $l_delimiter, ':upload_time' => $l_upload_time, ':impute_missing_values' => $l_impute_missing_values, ':log2_transform' => $l_log2_transform, ':organism_id' => $l_organism_id, ':standard_id_column' => $l_standard_id_column,
	  ':standard_id_type' => $l_standard_id_type, ':identifier' => $l_identifier, ':feature_descriptors' => $l_feature_descriptors, ':quant' => $l_quant, ':filter' => $l_filter, ':error_message' => $l_error_message, ':error_file' => $l_error_file, ':error_line' => $l_error_line, ':task' => $l_task);
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);

	//delete current set and current file
	$query = "DELETE FROM project_sets WHERE set_id=:set_id";
	$query_params = array(':set_id' => $l_set_id);
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);

	$query = "DELETE FROM project_conditions WHERE set_id=:set_id";
	$query_params = array(':set_id' => $l_set_id);
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);

	$query = "DELETE FROM project_replicates WHERE set_id=:set_id";
	$query_params = array(':set_id' => $l_set_id);
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);

	$query = "DELETE FROM project_files WHERE file_id=:file_id";
	$query_params = array(':file_id' => $l_file_id);
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);
	...
	$query_params = array(':project_id' => $projectID, ':identifier' => $set_id, ':identifier_type' => 'set_id', ':time' => date("Y-m-d H:i:s"));
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);

	//add upload fail to project activity
	$query = "INSERT INTO project_activity (project_id, activity, time, description) VALUES (:project_id, :activity, :time, :description)";
	$query_params = array(':project_id' => $l_project_id, ':activity' => "FAILURE", ':time' => date("Y-m-d H:i:s"), ':description' => "Statistical processing of data from '" . $l_og_file_name . "' failed. Site admins have been contacted.");
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);

	//set progress to 100 and failure to 1
	$query = "UPDATE process_queue SET process_failed=1, completed=1, running=0 WHERE process_id=:process_id";
...
{
	global $gerrstr, $gerrfile, $gerrline;
	$gerrstr = $errstr;
	$gerrfile = $errfile;
	$gerrline = $errline;
...
function CoefficientOfVariation($array, $is_log2=true)
{
	if($is_log2)
	{
		...
		$cv = (100 * (standard_deviation($tmp_array))/Mean($tmp_array));
		return ($cv);
	}
	else
	{
		...
		$cv = (100 * (standard_deviation($array))/Mean($array));
		return ($cv);
	}
}
function Mean($arr)
...
function tTestTwoSample($array1, $array2)
{
	...
	$σ₁ = standard_deviation($array1); $σ₂ =standard_deviation($array2);
...
function BCMean($arr)
{
	...
	$val = bcdiv($val, count($arr));
	return $val;
}
function BCStdDev($arr)
{
	if(count($arr)<=1)
	{
		return 0;
	...
	$mean = BCMean($arr);
	foreach ($arr as $entry) {
		$val = bcadd($val, bcpow(bcsub($entry, $mean), 2));
	}
	$val = bcdiv($val, count($arr)-1);
	return bcsqrt($val);
}

function BCTTestTwoSample($array1, $array2)
{
	if(empty($array1) || empty($array2)) { return 1;}
	if(count($array1)===0 || count($array2)===0) { return 1;}
	$μ₁ = BCMean($array1); $μ₂ = BCMean($array2); $n₁ = count($array1); $n₂ = count($array2);
	$σ₁ = BCStdDev($array1); $σ₂ =BCStdDev($array2);
	$Δ = 0;
	$num = bcsub($μ₁, $μ₂);
	$denomA = bcdiv(bcpow($σ₁,2), $n₁);
	$denomB = bcdiv(bcpow($σ₂,2), $n₂);
	$denom = bcadd($denomA, $denomB);
	$denom = bcsqrt($denom);
	if ($denom==0)
	{
		return 1;
	...
	$t = bcdiv($num, $denom);
	$v = bcadd(bcsub($n₁, 1), bcsub($n₂, 1));
	$pval = BCTCall(BCBuzz(abs($t), $v));
	return abs($pval);
}
...
function BCBuzz($t,$n)
{
	$pi=pi();
  	...
  	$rt = bcdiv($t, bcsqrt($n));
  	$fk=atan($rt);
	if($n==1) { return bcsub(1, bcdiv($fk,$pj2)); }
	$ek=sin($fk); $dk=cos($fk);

	if(($n%2)==1)
	{ 
		$val = 0;
		$val = BCZip(bcmul($dk, $dk), 2, bcsub($n,3), -1);
		$val = bcmul($val, $dk);
		$val = bcmul($val, $ek);
		$val = bcadd($fk, $val);
		$val = bcdiv($val, $pj2);
	...
	{ 
		$val = BCZip(bcmul($dk, $dk),1, bcsub($n,3), -1);
		$val = bcmul($ek, $val);
		return bcsub(1, $val);
		//return 1-$ek*zip($dk*$dk,1,$n-3,-1); 
	}
