...
uploadDataApp.controller('uploadCtrl', function($scope, $http, $filter, FileUploader, hotRegisterer, $rootScope, $location, $window, $interval) {
	...
	$scope.onlyNumbers = /^\d+$/;
	$scope.header = "";
	$scope.delimiters = [{delimiter:"Comma (',')", value:"COMMA"}, {delimiter:"Tab ('\\t')", value:"TAB"}, {delimiter:"Whitespace (' ')", value:"WHITESPACE"}, {delimiter:"Semicolon (';')", value:"SEMICOLON"}];
...
$scope.createTempTree = function(reps)
{
    ...
    $scope.saveData = function()
    {
    	...
    	$http({
    		...
    		data: $.param({
    			id: id, fd: fd, q: q, log2: log2, imp: imp, filter: filter, bi: bi, sn: sn, delim: delim, o: o, sc: sc, st: st
    		}),
    		headers: {'Content-Type': 'application/x-www-form-urlencoded'}
    	}).then(function success(response){
