...
//add POST variables to temporary file headers table (note: once the file has been validated you need to move this information to permanent file table so it cannot be deleted or overwritten).
	$argLine = "INSERT INTO project_temporary_file_headers (
		uploader_user_id,project_id,temporary_file_id,original_file_name,file_name,upload_time,identifier,feature_descriptors,quant,log2_transform,impute_missing_values,filter,branch_id,set_name,delimiter, organism_id,standard_id_column,standard_id_type)
VALUES (:uploader_user_id,:project_id,:temporary_file_id,:original_file_name,:file_name,:upload_time,:identifier,:feature_descriptors,:quant,:log2_transform,:impute_missing_values,:filter,:branch_id,:set_name,:delimiter, :organism_id,:standard_id_column,:standard_id_type)";
$query_params = array(
	...
	':impute_missing_values' => $impute,':filter' => $filter,':branch_id' => $branch_id,':set_name' => $set_name,':delimiter' => $delimiter, ':organism_id' => $organism_id, ':standard_id_column' => $standard_identifier_column_header, ':standard_id_type' => $standard_identifier_type
	); 
try
{
...
//check that the file exists 
	...
	{
		header('HTTP/1.1 500 Internal Server Error');
		throw new RuntimeException($full_file_path);
		die();
	}
