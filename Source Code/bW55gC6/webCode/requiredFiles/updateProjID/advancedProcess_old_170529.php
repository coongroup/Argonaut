...
function HandleErrorExit($process_id, $set_id, $projectID, $db)
{
	...
	//Check if the set exists -- if yes then the upload should have worked and you should mark this as a failure
	//Perhaps later you can report this to the uploading user with a message about why the upload failed. Later work though.
	$query = "SELECT set_name FROM project_sets WHERE set_id=:set_id";
	$query_params = array(':set_id' => $set_id);
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);
	$row = $stmt->fetch();
	...
	//update the process_queue table
	$query = "UPDATE process_queue SET running=:running, complete=:completed, progress=:progress, process_failed=:failed, task_completion_time=:time";
	$query_params = array(':running' => $running, ':completed' => $completed, ':progress' => $progress, ':failed' => $failed, ':time' => date("Y-m-d H:i:s"));
	try{
