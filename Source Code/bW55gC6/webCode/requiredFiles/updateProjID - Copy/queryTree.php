...
{
	array_push($list, array('name'=>$entry['project_id'], 'value'=>$entry['project_name'], 'parent' => 'null', 'x0' => 0, 'y0' => 0, 'x' => 0, 'y' => 0));
}
...
foreach($row as $entry)
{
	array_push($list, array('name'=>$entry['branch_id'], 'value'=>$entry['branch_name'], 'parent' => $entry['project_id'], 'x0' => 0, 'y0' => 0));
}
...
foreach($row as $entry)
{
	array_push($list, array('name'=>$entry['set_id'], 'value'=>$entry['set_name'], 'parent' => $entry['branch_id'], 'x0' => 0, 'y0' => 0));
}
...
foreach($row as $entry)
{
	array_push($list, array('name'=>$entry['condition_id'], 'value'=>$entry['condition_name'], 'parent' => $entry['set_id'], 'x0' => 0, 'y0' => 0));
}
...
foreach($row as $entry)
{
	array_push($list, array('name'=>$entry['replicate_id'], 'value'=>$entry['replicate_name'], 'parent' => $entry['condition_id'], 'x0' => 0, 'y0' => 0));
}
