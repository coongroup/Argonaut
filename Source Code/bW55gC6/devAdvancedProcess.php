...
function AddReplicateDataToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings)
{
	...
	fclose($new_file);
	$query = "LOAD DATA INFILE 'C:/xampp/htdocs/Projects/WebDeposition_v4/DV/" . $projectID . "/" . $set_id . "_data_replicate_data.txt' INTO TABLE data_replicate_data FIELDS TERMINATED BY '\t' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' (project_id, unique_identifier_id, quant_value, quant_value_log2, quant_value_raw, replicate_id, condition_id, set_id, branch_id, file_id, is_imputed, standard_molecule_id)";
...
function AddConditionDataToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings)
{
	...
	fclose($new_file);
	$query = "LOAD DATA INFILE 'C:/xampp/htdocs/Projects/WebDeposition_v4/DV/" . $projectID . "/" . $set_id . "_data_condition_data.txt' INTO TABLE data_condition_data FIELDS TERMINATED BY '\t' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' (project_id, unique_identifier_id, avg_quant_value, all_quant_values, std_dev_quant_value, cv_quant_values, num_quant_values, condition_id, set_id, branch_id, file_id, standard_molecule_id)";
	$stmt = $db->prepare($query, array(PDO::MYSQL_ATTR_LOCAL_INFILE => true));
	//$result = $stmt->execute();
...
function AddDataDescriptiveStatisticsToDatabase($condition_dict, $projectID, $set_id, $branch_id, $file_id, $db, $standard_id_mappings)
{
	...
	fclose($new_file);
	$query = "LOAD DATA INFILE 'C:/xampp/htdocs/Projects/WebDeposition_v4/DV/" . $projectID . "/" . $set_id . "_data_descriptive_statistics.txt' INTO TABLE data_descriptive_statistics FIELDS TERMINATED BY '\t' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' (project_id, file_id, set_id, branch_id, condition_id, unique_identifier_id, quant_val, fold_change_mean_norm, fold_change_control_norm, p_value_mean_norm, p_value_control_norm, std_dev, bonferroni_p_value_control_norm, bonferroni_p_value_mean_norm, fdr_p_value_control_norm, fdr_p_value_mean_norm, standard_molecule_id)";
	$stmt = $db->prepare($query, array(PDO::MYSQL_ATTR_LOCAL_INFILE => true));
	//$result = $stmt->execute();
...
function HandleErrorExit($process_id, $set_id, $projectID, $db)
{
	...
	$mail->From = "nickwiecientest@gmail.com";
	$mail->FromName = "Nick";
	$mail->Host = "smtp.gmail.com"; // specif smtp server
	$mail->SMTPSecure= "ssl"; // Used instead of TLS when only POP mail is selected
	$mail->Port = 465; // Used instead of 587 when only POP mail is selected
	$mail->SMTPAuth = true;
	$mail->Username = "nickwiecientest@gmail.com"; // SMTP username
	$mail->Password = "Nwapril8\$"; // SMTP password
