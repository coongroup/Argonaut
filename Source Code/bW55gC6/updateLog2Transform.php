...
foreach ($transformData as $entry)
{
	...
	if ($current_data['log2_transform']!==$entry['log2'])
	{
		$current_data['log2_transform'] = $entry['log2'];
		array_push($update_data, $current_data);
	}
	$current_data['log2_transform']!==$entry['log2'] ?  : null;
}
...
foreach ($update_data as $entry) {
	...
	//Update file id Pretty sure this is unnecessary
	...
	$control_description = "Updated log2 transformation settings in '" . $set_name . "' (Branch: " . $branch_name . ")";
	$query = "INSERT INTO project_activity (project_id, activity, time, description) VALUES (:project_id, :activity, :time, :description)";
	$query_params = array(':project_id' => $projectID, ':activity' => 'LOG2 UPDATE', ':time' => $time, ':description' => $control_description);
	$stmt = $db->prepare($query);
	$result = $stmt->execute($query_params);
	...
	$query_params = array(':user_id' => $entry['uploader_user_id'], ':project_id' => $projectID, ':task_params' => '', ':task' => 'LOG2', ':set_id' => $new_set_id, ':branch_id' => $branch_id,
...
if(!$row)
{
	...
	curl_setopt($ch, CURLOPT_URL, "127.0.0.1/bW55gC6/process.php");
...
$data = array("result"=>true, "message"=>"Data from the " . $set_line . " '" . $file_name_string . "' is being reprocessed to reflect updated log2 transformation settings. Data from ".$file_line." will become available once processing routines have completed.");
echo(json_encode($data));
return;
